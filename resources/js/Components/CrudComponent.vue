  ...
  <AuthenticatedLayout>
    <div class="card">
      <Toolbar class="">
        <template #start>
          ...
          <ButtonGroup class="mr-2">
            <Link
              v-if="vueProps.config.indexRoute"
              :href="vueProps.config.indexRoute"
            >
              <Button
                label="All Items"
                icon="pi pi-list"
                :class="{ 'border-bottom-2': !isTrashedPage }"
                text
              />
            </Link>
            <Link
              v-if="vueProps.config.bulkRestoreRoute"
              :href="vueProps.config.indexRouteTrashed"
            >
              <Button
                label="Trashed"
                icon="pi pi-ban"
        ...
        <template #end>
          <Button
            v-if="vueProps?.config?.exportRoute"
            label="Export"
            class="mx-2"
            icon="pi pi-upload"
            severity="secondary"
      ...
      <DataTable
        ...
        :value="vueProps.items.data"
        data-key="id"
        :paginator="true"
        :rows="15"
        :filters="filters"
        :total-records="vueProps.items.total"
        :lazy="true"
        paginator-template="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
        :rows-per-page-options="[5, 10, 25]"
        :current-page-report-template="`Showing {first} to {last} of {totalRecords} ${vueProps.config.resource}`"
        resizable-columns
      ...
      >
        <template #empty>
          <div class="p-4 text-center">
            <p class="text-lg">
              No {{ vueProps.config.resource }} found.
            </p>
          </div>
        </template>
        <template #header>
    ...
    <Dialog
      ...
      maximizable
      :style="{ width: formWidth ?? '60vw' }"
      :header="`${vueProps.config.modelRaw} Details`"
      pt:mask:class="backdrop-blur-sm"
    >
      <slot
        name="form"
        v-bind="{ submitted, statuses, handlePhotoUpload, photoPreview, resolveImagePath }"
      />
