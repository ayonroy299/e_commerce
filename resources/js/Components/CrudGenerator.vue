  ...
  <div>
    ...
    >
      ...
      <DataTable
        ...
        :current-page-report-template="`Showing {first} to {last} of {totalRecords} ${props.config.entity}`"
        @page="handlePagination($event, props.config.endpoints.list, props.config.entity)"
      >
        <template #header>
          <div class="flex flex-wrap gap-2 items-center justify-between">
            <h1 class="text-3xl">
              {{ props.config.title || 'Manage Items' }}
            </h1>
            <IconField>
              <InputIcon>
              ...
              <div>
                ...
                <template v-else-if="field.type === 'file'">
                  <FileUpload
                    ...
                    :auto="true"
                    :accept="field.accept || '*'"
                    :choose-label="field.chooseLabel || 'Choose File'"
                    class="w-full"
                    @select="(e) => handleFileUpload(e, field.name)"
                  />
                  <div
                    v-if="field.isImage && (form[field.name] || photoPreview)"
                    class="mt-2"
                  >
                    <img
                      :src="photoPreview || resolveImagePath(form[field.name])"
                      alt="Preview"
                ...
                <small
                  v-if="submitted && field.required && !form[field.name]"
                  class="text-red-500"
                >
